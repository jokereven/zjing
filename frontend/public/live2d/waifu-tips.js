...
function getCookie(e) {
	for (
		var t = e + 'acai' + '=', i = document.cookie.split(';'), o = 0;
		o < i.length;
		o++
	) {
		for (var a = i[o]; ' ' == a.charAt(0); ) a = a.substring(1, a.length);
...
function initWidget(config, apiPath = '/') {
	...
	toggle.addEventListener('click', () => {
		'Hide' == getCookie('live2d')
			? setTimeout(function () {
					document.body.clientWidth > 860 &&
						($('.hide-live2d').css('bottom', '66px'),
						$(
							'.save-live2d, .switch-live2d, .live2d-pio, .live2d-tia'
						).addClass('hide-live2d-tool')),
